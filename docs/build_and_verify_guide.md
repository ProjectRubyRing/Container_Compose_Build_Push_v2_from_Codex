...
| 23 | JMeter 性能試験の実行と実行状況の確認 (compose へ追加された jmeter コンテナを `docker compose --profile loadtest run --rm jmeter run <テスト計画>` として呼び出し、テスト計画 (`.jmx`) に設定されているスレッド数・Ramp-Up 期間・ループ回数を上書きして実行する。結果 (`result.jtl`) と HTML ダッシュボードは**レポートファイルと同じディレクトリ**へ出力する) | (`--keep-container-mode logs` の「JMeter 性能試験を実行」「JMeter 実行状況を確認」。サービス名・profiles は `--jmeter-service` / `--jmeter-profile`、負荷条件は `--jmeter-threads` / `--jmeter-rampup` / `--jmeter-loops`、出力先は `--jmeter-report-dir`、別端末からの確認は `--jmeter-status`) |
...
ビルド出力は常に控えておき、サービス単位へ切り分けたビルドログを全量レポートとは
別のファイルへ書き出します。ビルドエラーで終了する実行では、各サービスの
ビルドログを画面へ全量表示してから終了します (5.2-0-2 参照)。
...
### 4.6-3 JMeter 性能試験 (jmeter コンテナの呼び出し)

| オプション | 値の形式 | 既定値 | 説明 |
| --- | --- | --- | --- |
| `--jmeter-service NAME` | Compose サービス名 | `jmeter` | JMeter 実行コンテナのサービス名 (→ 5.4-5) |
...
| `--jmeter-report-dir DIR` | ディレクトリパス | (`--report-dir` → 一時ディレクトリ) | 結果 (`result.jtl` / `report/index.html` / `jmeter.log`) の出力先 |
...
| `--jmeter-status` | フラグ | — | 実行状況を表示して終了する (ビルド・起動・後始末をしない) |
...
### 5.2-0-2 サービス別ビルドログ (`base` / `frontend` / `backend`)

`docker compose build` の出力は、ベースサービスを先行ビルドしたあと残りを並列
ビルドするため (3.2 参照)、1 つの画面へ複数サービス分が混ざって流れます。
ビルドエラーで終了する実行では `--keep-container-mode logs` の対話ダイアログまで
進めないため、後からログを読み返す手段もありません。そこでビルド出力は常に
控えておき、サービス単位へ切り分けたビルドログを残します。オプションの指定は
不要で、既定で動きます。
...
| タイミング | 動作 |
| --- | --- |
| ビルド中 | ビルド 1 回分の出力を一時ファイルへ控える (画面表示は従来どおり) |
| ビルド終了時 | サービス単位へ切り分け、サービスごとに 1 ファイルずつ書き出して出力先を画面へ表示 |
| **ビルドエラーで終了するとき** | **上記に加えて、各サービスのビルドログを行数の制限なしで画面へ全量表示** |
...
同名のファイルが既にある場合は連番を足し、前回の結果を上書きしません。
ビルド引数や環境変数が出力へ混ざることがあるため、ファイルは `umask 077` で
作成します。出力先は全量レポートの `[1] ビルド結果` の「ビルドログ」欄にも
記録します。

#### 切り分けの規則

BuildKit は `--progress=plain` で、行頭へ `#<番号> [<サービス名> <段>]` を出します。

```text
#12 [frontend build 3/8] RUN /opt/build.sh
#12 0.412 Downloading dependencies ...
#12 DONE 12.4s
```

- `#<番号>` と `<サービス名>` の対応を覚えておき、同じ番号で続く行 (`RUN` の出力や
  `DONE` 行) も同じサービスへ振り分けます。
- サービス名として受け付けるのは、そのビルドの対象サービス名と完全に一致する
  ものだけです。`[internal]` のような見出しをサービス名と取り違えません。
- 対象サービスが 1 つだけのビルド (ベースサービスの先行ビルドなど) は、出力の
  全行をそのサービスのものとして扱います。
- tty 形式 (`--progress=auto`) の `=> [frontend 2/6] RUN ...` と、それに続く
  `=> => # ...` は、直前に見出しが出たサービスへ続けて振り分けます。
- どのサービスにも紐付かなかった行 (`ERROR: failed to solve` など) は「共通の
  出力」として、各サービスのログの末尾へ添えます。どのファイルを開いても失敗
  そのものの行を読めるようにするためです。

#### ビルドまで到達しなかったサービス

`base` の先行ビルドで失敗した実行では、`frontend` / `backend` のビルドまで到達
しません。この場合も見出しとファイルは必ず作り、`(ビルドが始まる前に終了したか、
今回のビルド対象ではないため、このサービスのビルド出力はありません)` と明示
します。「ログが空」なのか「そこまで進んでいない」のかを、画面からもファイル
からも区別できるようにするためです。対象は `base` / `frontend` / `backend` を
サービス名に含むサービスで、`--frontend-context` などの対象判定と同じ規則です。

#### 制約

- `--dry-run` では出力しません (ビルドを実行しないため)。
- 一時ファイルを作れない環境では、警告を出したうえでビルドをそのまま続けます
  (ログを残せないことでビルドを止めません)。全量レポートには、出力しなかった
  理由を記録します。
- 記録のため、ビルドの標準エラー出力は標準出力へまとめます。ビルド監視を行う
  既定の実行では以前からこの扱いで、`--no-build-watchdog` を指定した実行も
  同じ扱いに揃えています。
...
「マウントできている」ことと「置いたものが相手から同じように見える」ことは別物で、
次の食い違いはコンテナを 1 つ覗いただけでは分かりません。
...
この操作は、ローカルでバッチサーバーを肩代わりする**偽装サービス**
(このリポジトリの姉妹構成では `batch-mock` サービス。実装は
`Container_Compose_file/compose/batch-mock/`) から EFS へ書き、
同じ EFS をマウントする全コンテナからどう見えるかを突き合わせます。

| 項目 | 内容 |
| --- | --- |
| 表示条件 | `batch-mock` サービスが起動しており、選択サービスが偽装バッチサーバーと**同じボリューム (識別キー)** をマウントしていること。偽装バッチサーバー自身を選ぶと全コンテナへの伝播をまとめて確認する |
| 表示対象の判定 | 偽装バッチサーバーのコンテナ内 CLI へ `mounts` を問い合わせて EFS のマウント先を得たうえで、`docker inspect` のマウント一覧 (named volume 名 / bind の実体パス) が一致するかで決める。サービス名をスクリプトへ固定で持たないため、EFS を共有するサービスを増やせば表示対象も自動で増える |
| CLI の解決 | コンテナの環境変数 `BATCH_MOCK_CLI` → 既定 `/opt/batch-mock/efs-propagation.sh`。CLI はコンテナ内の POSIX sh で動くため、ホスト側にも コンテナ側にも Python は不要 (alpine でそのまま動く) |
| (1) 偽装バッチサーバーの状態 | 起動状態・自身の `healthcheck`・連続失敗回数・再起動回数・起動時刻・**実行 `uid:gid`**・EFS マウント (マウント先 / `rw` `ro` / ボリューム名)。`running` でない / `unhealthy` / `uid:gid` が `BATCH_MOCK_EFS_UID_GID` (既定 `6301:6302`) と違うときは警告する |
| (2) 作成 | 各マウント先の `batch-mock/` 配下へ、ディレクトリ (mode 2775)・ファイル (mode 664)・**相対**シンボリックリンク (ファイル用 / ディレクトリ用) を作る。続けて全コンテナからシンボリックリンク経由で読み、内容・リンク先・所有者・mode を突き合わせる |
| (3) 書き換え | シンボリックリンク経由で追記し、更新が全コンテナへ伝わることを確認する (**複製ではなく本当に共有されているか**はここで分かる) |
| (4) 後始末 | 作ったものを消し、全コンテナから消えたことを確認する。消したのにまだ読める場合は共有ではなく複製として警告する |
| 判定表示 | `OK` (作成・書き換え・削除のすべてが全コンテナへ反映) / `NG` (1 つでも反映されていない) |
| 終了扱い | `NG` は診断結果としてそのまま操作選択へ戻る。偽装バッチサーバーから状態を取得できない、または EFS へ書き込めない場合だけヘルパー失敗として扱う |

コンテナごとの行は `[OK]` / `[NG]` で表示し、`NG` の理由は次のように分かれます。

| 表示 | 意味 | よくある原因 |
| --- | --- | --- |
| `... が見えません` | パスそのものが無い | そのコンテナに EFS をマウントし忘れている |
| `... のリンク先を解決できません` | シンボリックリンクはあるが実体へ届かない | リンクを**絶対パス**で張っており、マウント先が異なるコンテナでは解決できない |
| `... を読めましたが印が見つかりません` | 読めたが内容が古い | 共有ボリュームではなくコンテナごとの複製になっている |

印 (マーカー) は実行ごとに一意 (`bv-<日時>-<PID>`) にするため、前回の残りを読んで `OK` と
誤判定することはありません。偽装バッチサーバーとマウント先が異なるコンテナ
(例: `efs-mock` の `/mnt/efs/logs`) でも、同じボリュームであればマウント先を読み替えて
確認します。1 コンテナが複数のボリュームを持つ場合も、ボリュームごとに 1 回だけ確認します。

偽装バッチサーバーの Compose サービス名や、EFS を初期化する `uid:gid` を変えている場合は、
スクリプト冒頭の `BATCH_MOCK_SERVICE` (既定 `batch-mock`) と
`BATCH_MOCK_EFS_UID_GID` (既定 `6301:6302`) を合わせてください。

#### bash を持たないコンテナへの接続

偽装 EFS (`efs-mock`) や偽装バッチサーバー (`batch-mock`) のような `alpine` / `busybox`
ベースのコンテナには `/bin/bash` がありません。`/bin/bash` 決め打ちの `docker exec` は
`exec: "/bin/bash": stat /bin/bash: no such file or directory` となって接続できないため、
接続時に**実在するシェルを探してから**接続します。
...
解決結果はコンテナ ID (と `-u` の指定) をキーに覚えるため、同じコンテナへ何度接続しても
探索は 1 回だけです。

`sh` へ切り替えたときは、その旨と「bash 固有の書き方 (配列、`[[ ]]`、シェル関数の
`export`) は使えない」ことを接続前に表示します。`sh` 側の簡易 `tree` も
`-a` / `-d` / `-f` / `-L 深さ` / `--noreport` に対応し、シンボリックリンクは参照先を
併記します (`-a` の並び順だけ、ドット項目が先になります)。POSIX sh には `export -f` が無く
対話シェルは子プロセスになるため、簡易 `tree` は必ず書き込めるディレクトリへファイルとして
置いて `PATH` の先頭へ足します (どこも書けない場合は `tree` 無しで対話シェルを始めます)。

シェルを一切持たないイメージ (distroless など) では、探索したパスを挙げたうえで
「`docker exec` による対話接続はできない」と伝えて操作選択へ戻ります。
...
「root ユーザで bash へ接続」、デプロイエラー時の調査モードを含む) では、
接続の直前に `tree` の有無を調べ、
...
### 5.4-5 JMeter 性能試験 (jmeter コンテナの呼び出し)

compose へ追加された **jmeter サービス**は、常駐サービスではなく
**「実行するたびに起動して終わるジョブ」**として定義されています
(`profiles: ["loadtest"]`)。そのため `docker compose up` では起動せず、
起動確認の対象にもならず、`docker compose ps` にも現れません。

このスクリプトは `--keep-container-mode logs` の対話操作から、その jmeter
コンテナを `docker compose --profile loadtest run --rm jmeter run <テスト計画>`
として呼び出します。

#### サービス選択の一覧に出る

jmeter サービスが compose に定義されていれば、**起動していなくても**
サービス選択の一覧へ出ます (起動中のサービスの後ろに置き、既存の番号は動かしません)。
...
jmeter を選ぶと、サービス操作メニューの末尾へ 2 つの操作が増えます
(他のサービスでは増えないため、既存構成の操作番号は変わりません)。
...
テスト計画の一覧と、コンテナ側の既定値は**コンテナ自身に答えさせます**
(`JMETER_PLANS_DIR` などのパスは compose 側で決まるため、ホストで推測しません)。
選んだ `.jmx` はホストへ取り出し、設定されている負荷条件を読んで表示します。
...
上書きは 2 段構えで行います。**どちらの書き方の `.jmx` でも同じ結果になる**ようにするためです。

| | 何をするか | どの書き方に効くか |
| --- | --- | --- |
| (A) 環境変数 | `-e JMETER_THREADS` / `-e JMETER_RAMPUP` / `-e JMETER_LOOPS` を `docker compose run` へ渡す。jmeter コンテナ側でこれが `-Jthreads` / `-Jrampup` / `-Jloops` になる | `${__P(threads,10)}` のように書かれたテスト計画 |
| (B) 複製の書き換え | 該当箇所だけを書き換えた**複製**を一時ディレクトリへ作り、読み取り専用でマウントして実行する (`JMETER_PLANS_DIR` を差し替える) | GUI で作った `.jmx` のように**数値が直接書かれている**テスト計画 |

**元の `.jmx` は一切変更しません。** 書き換えるのは通常のスレッドグループの
`ThreadGroup.num_threads` / `ThreadGroup.ramp_time` と、**その中の**
`LoopController.loops` だけです。配下のループコントローラや
setUp / tearDown スレッドグループは、同じプロパティ名を持っていても対象にしません
(取り違えると試験の中身が変わってしまうため)。

上書き後の値は実行前に表示し、実際に使ったテスト計画の複製は結果ディレクトリへ
`plan.jmx` として残ります (後から同じ条件で再現できます)。

#### 結果はレポートファイルと同じディレクトリへ出る

コンテナ側の結果出力先 (`/results`) ではなく、**レポートファイルと同じディレクトリ**を
コンテナへマウントして書かせます。`--report-dir` を指定していればその配下、
指定が無ければ一時ディレクトリで、`--jmeter-report-dir` で変更できます。
...
> 別端末へ渡すときやディレクトリを消すときは、所有者にご注意ください。

#### 実行方法 (背面 / 前面) と実行状況の確認

性能試験は数分〜数十分かかるため、**背面 (バックグラウンド) 実行が既定**です。
開始するとすぐ操作の選択へ戻れます。前面実行を選べば、終わるまで出力を画面へ流します。

```
実行方法を選択してください:
  1) 背面で実行し、すぐ操作の選択へ戻る (既定。進捗は「JMeter 実行状況を確認」で見る)
  2) 前面で実行し、終わるまで待つ (出力をそのまま画面へ出す)
  0) 実行を中止
```

進捗と結果は、同じメニューの「JMeter 実行状況を確認」か、**別の端末から**
`--jmeter-status` で確認できます。`--jmeter-status` はビルドも起動も後始末も行わず、
状況を表示して終了します (状況を見ただけでコンテナが消えることはありません)。

```bash
# 別端末から、走っている性能試験の進捗を見る
./build_and_verify.sh --jmeter-status --report-dir /var/tmp/reports
```

表示するのは、結果ディレクトリの実行一覧 (新しい順)、実行中の JMeter コンテナ
(`docker ps`)、最新実行の進捗 (JMeter の `summary` 行)、`result.jtl` と
HTML レポートの出力先、一次集計です。実行中のまま残っている記録は、
コンテナが生きているかどうかで裏を取ります。

> 対話操作をすべて終える (「0) 対話操作を終了」) と、既定では
> `compose down` → 未使用リソースの完全クリアまで進みます。背面実行が残っていると
> 試験は途中で終わり結果も残らないため、**終了する前に完了を待つかどうかを確認します**。
...
| `--report-dir/build_and_verify_<日時>_build_log_<サービス名>.txt` | ビルドを実行した実行では常時 (`--report-dir` が無ければ一時ディレクトリ) | サービス別ビルドログ。`docker compose build` の出力のうちそのサービスの行を全量。末尾にサービスを特定できなかった「共通の出力」を添える (5.2-0-2 参照) |
...
| セクション | 内容 | コンテナ未起動時 |
| --- | --- | --- |
| `[1] ビルド結果` | 結果 / 詳細 / イメージ情報 / ビルド監視 / サービス別ビルドログの出力先 / 既存コンテナ点検 / `コピー取込検証` の要約 / 保存ポリシー | 記録される |
...
# 10-4) 起動後に JMeter で性能試験を行う (対話で計画とスレッド数 / Ramp-Up / ループを指定)
#       結果 (result.jtl) と HTML レポートは --report-dir と同じディレクトリへ出る
./build_and_verify.sh --verify-startup --keep-container-mode logs --report-dir ./reports

# 10-5) 負荷条件をあらかじめ決めておき、対話での入力を省く
./build_and_verify.sh --verify-startup --keep-container-mode logs --report-dir ./reports \
  --jmeter-plan sample-frontend.jmx --jmeter-threads 200 --jmeter-rampup 200 --jmeter-loops -1 \
  --jmeter-target frontend --jmeter-duration 600

# 10-6) 背面で走らせた性能試験の進捗を、別の端末から確認する
